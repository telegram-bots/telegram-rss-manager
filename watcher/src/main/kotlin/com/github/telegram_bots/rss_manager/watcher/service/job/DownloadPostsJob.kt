...
{
    override fun apply(channel: Channel): Flowable<TLMessage> = Flowable.range(channel.lastPostId + 1, batchSize)
            .toList()
            .map { TLIntVector().apply { addAll(it) } }
            .zipWith(Single.just(TLInputChannel(channel.telegramId, channel.hash)))
            .flatMap { (ids, channel) -> client.channelsGetMessages(channel, ids) }
            .flattenAsFlowable(TLAbsMessages::messages)
            .ofType()
}
